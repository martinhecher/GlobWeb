 ...
 */
	var Text = (function() {
		...
		var generateImageData = function(text, textColor) {
			if (!canvas2d) initialize();

			var fillColor = textColor;
			if (!fillColor) fillColor = '#fff';
			else if (fillColor instanceof Array) fillColor = FeatureStyle.fromColorToString(textColor);
 ...
 */
	var EffectRenderer = function(tileManager) {
	...
	";
		...
		var vertices = new Float32Array([-0.5, -0.5, 0.0, -0.5, 0.5, 0.0,
		0.5, 0.5, 0.0,
		0.5, -0.5, 0.0]);
 ...
 */
	EffectRenderer.prototype._buildDefaultTexture = function(bucket) {
 ...
 */
	EffectRenderer.prototype._buildTextureFromImage = function(bucket, image) {
 ...
 */
	EffectRenderer.prototype.addGeometry = function(geometry, layer, style) {
		if (style) {
 ...
 */
	EffectRenderer.prototype.removeGeometry = function(geometry, layer) {
 ...
 */
	EffectRenderer.prototype.getOrCreateBucket = function(layer, style) {
 ...
 */
	EffectRenderer.prototype.render = function() {
		...
		for (var n = 0; n < this.buckets.length; n++) {
			...
			if (bucket.texture == null || bucket.points.length == 0 || !bucket.layer._visible || bucket.layer._opactiy <= 0.0) continue;
			...
			for (var i = 0; i < bucket.points.length; ++i) {
				...
				if (scaleInKm > bucket.style.pointMaxSize) continue;
	...
	VectorRendererManager.registerRenderer({
		creator: function(globe) {
			return new EffectRenderer(globe.tileManager);
		},
		canApply: function(type, style) {
			var flag = (type == "Point" && (style.renderer == "pulsar"));
			if (flag) {
				console.log("[EffectRenderer] selected");
			}
			return flag;
		}
	});
